...
```{r trendTableDaily}
if(nrow(gw_level_dv) > 0){
  val_col <- paste("X", parameterCd, statCd, sep = "_")
  approve <- paste0(val_col, "_cd")
  
  gw_monthly <- monthly_mean(gw_level_dv[grepl("A", gw_level_dv[[approve]]),], 
                             value_col = paste("X",
                                               parameterCd, 
                                               statCd, sep = "_"))
  gw_monthly$Approved <- "A"
  trend_result <- kendall_test_5_20_years(NULL,
                                          gw_monthly, 
                                          seasonal = TRUE, 
                                          date_col = "mid_date", 
                                          value_col = "mean_va", 
